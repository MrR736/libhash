...
#define S512_224(value, bits) (((value) >> (bits)) | ((value) << (64 - (bits))))
#define SHA512_224_CH(x, y, z) (((x) & (y)) ^ (~(x) & (z)))
#define SHA512_224_MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))
#define SHA512_224_BSIG0(x) (S512_224((x), 28) ^ S512_224((x), 34) ^ S512_224((x), 39))
#define SHA512_224_BSIG1(x) (S512_224((x), 14) ^ S512_224((x), 18) ^ S512_224((x), 41))
#define SHA512_224_SSIG0(x) (S512_224((x), 1) ^ S512_224((x), 8) ^ ((x) >> 7))
#define SHA512_224_SSIG1(x) (S512_224((x), 19) ^ S512_224((x), 61) ^ ((x) >> 6))
...
static const uint64_t SHA512_224_K[80] = {
...
static inline void Sha512_224TransformFunction(Sha512_224Context* context,const uint8_t* data) {
	uint64_t S[8]; uint64_t W[80];
	uint64_t t1, t2; int i;
	...
	for (i = 16; i < 80; ++i) {
		W[i] = SHA512_224_SSIG1(W[i - 2]) + W[i - 7] + SHA512_224_SSIG0(W[i - 15]) + W[i - 16];
	}
	for (i = 0; i < 80; ++i) {
		t1 = S[7] + SHA512_224_BSIG1(S[4]) + SHA512_224_CH(S[4], S[5], S[6]) + SHA512_224_K[i] + W[i];
		t2 = SHA512_224_BSIG0(S[0]) + SHA512_224_MAJ(S[0], S[1], S[2]);
...
LIBHASH_INLINE_API void Sha512_224Initialise(Sha512_224Context* context) {
	memcpy(context->state, SHA512_224_IV, sizeof(SHA512_224_IV));
	context->length = 0;
	context->curlen = 0;
}

LIBHASH_INLINE_API void Sha512_224Update(Sha512_224Context* context,const void* input,uint32_t length) {
	const uint8_t* data = (const uint8_t*)input;
	while (length > 0) {
		uint32_t n = SHA512_224_BLOCK_SIZE - context->curlen;
		if (n > length) n = length;
		memcpy(context->buf + context->curlen,data,n);
		context->curlen += n;
		data += n;
		length -= n;
		if (context->curlen == SHA512_224_BLOCK_SIZE) {
			Sha512_224TransformFunction(context,context->buf);
			context->length += (uint64_t)SHA512_224_BLOCK_SIZE * 8;
			context->curlen = 0;
		}
	}
}

LIBHASH_INLINE_API void Sha512_224Finalise(Sha512_224Context* context,SHA512_224_HASH* digest) {
	uint64_t bit_length;
	uint32_t i;
	 ...
	 *
	 * This implementation tracks the low 64 bits, matching
	 * the length representation used by the existing SHA-512
	 * implementation.
	 */
	for (i = 0; i < 8; ++i) context->buf[112 + i] = 0;
	for (i = 0; i < 8; ++i) context->buf[120 + i] = (uint8_t)(bit_length >> (56 - i * 8));
	Sha512_224TransformFunction(context,context->buf);
	for (i = 0; i < SHA512_224_HASH_SIZE; ++i) digest->bytes[i] = (uint8_t)(context->state[i / 8] >> (56 - (i % 8) * 8));
	memset(context, 0, sizeof(*context));
}

LIBHASH_INLINE_API void Sha512_224Calculate(const void* input,uint32_t length,SHA512_224_HASH* digest) {
	Sha512_224Context context;
	Sha512_224Initialise(&context);
	Sha512_224Update(&context, input, length);
	Sha512_224Finalise(&context, digest);
}
